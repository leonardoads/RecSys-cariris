...
data = data.frame(data, bought, clicked, soldability, is_buy)

colnames(data) <- c("SESSION", "DAY", "MONTH", "YEAR", "TIME", "ITEM", "CATEGORY", "BOUGHT", "CLICKED", "SOLDABILITY", "IS_BUY")
...
write(capture.output(CrossTable(data_train$IS_BUY, pred, prop.chisq = FALSE, prop.c = FALSE, prop.r = FALSE, dnn = c('actual default', 'predicted default'))), "/home/ubuntu/dev/RecSys-cariris/Data/reports/basic+clicked+bought+soldability.txt")
